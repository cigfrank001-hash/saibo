...
按品牌输出最小意图清单（5 词起步包），可直接对接迪光 GEO 报价表 V1.3。
...
| 意图类型 | 锚点 | 报价 | 备注 |
|---------|------|------|------|
| 品牌意图 | 品牌词 | ¥1,500/意图 | V0.5 沿用，按平台数叠加 |
| 行业意图 | 行业/场景/品类/其他 | **平台阶梯** ¥6,000 起 | V1.3 拍板，全/半/全/半...阶梯 |
| 区域意图 | 地域导向 | **平台阶梯** ¥6,000 起 | V1.3 拍板，同行业意图 |

### V1.3 按意图类型定价（V1.3 拍板）

**3 类意图，类型独立定价 + 第 2+ 平台统一半价**：

| 意图类型 | 基础价（第 1 平台） | 半价（第 2+ 平台） | 2 平台 | 3 平台 |
|---------|------------------|------------------|--------|--------|
| **品牌意图** | ¥1,500/月 | **无阶梯**（固定） | ¥1,500 | ¥1,500 |
| **行业意图** | ¥6,000/月 | ¥3,000/月 | **¥9,000** | **¥12,000** |
| **区域意图** | ¥3,000/月 | ¥1,500/月 | **¥4,500** | **¥6,000** |

**阶梯公式**：`calc_platform_price(N) = 基础价 + (N-1) × 半价`

**品牌意图沿用 V0.5 模型**：¥1,500/月/平台，无阶梯。
...
```bash
# 1 平台（默认）
python3 keyword_pick.py --input Stage3.xlsx --collect --results Stage4.json

# 2 平台（每意图 ¥9,000）
python3 keyword_pick.py --input Stage3.xlsx --collect --results Stage4.json --platforms 2
```
...
- **V1.2**（2026-06-29）行业意图 ¥3,000/月/平台
- **V1.3**（2026-06-30）行业意图 = 平台阶梯：第 1 平台 ¥6,000，第 2+ 全部半价 ¥3,000
