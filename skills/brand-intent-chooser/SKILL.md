...
description: 品牌意图选词助手。输入品牌+行业+产品，按 4 阶段流程输出 N 个最小意图单元（每个 = 1 角色+1 场景+1 阶段+5 词），对接迪光 GEO 报价表 V1.3（行业意图阶梯 ¥6,000 起步，区域意图阶梯 ¥3,000 起步，第 2+ 平台半价）。用于"按品牌出最小意图清单"或"为新 GEO 客户准备 5 词起步包"。
---
...
### Stage 2 - 意图发散
- **半自动**：生成 **3 类意图**头脑风暴 prompt，用户手动复制到 5 家 AI 平台（DeepSeek/豆包/Kimi/文心/通义）跑
- 收集 5 家平台结果后，自动合并去重
- 输出：候选意图池 md
...
### Stage 2 输出示例（**3 类意图** × 5 个）
- **品牌意图**：「蔚来 ET5 价格」「蔚来 ES8 续航」「蔚来 ET7 配置」「蔚来换电服务」「蔚来 ET5 试驾」
  - V1.3 合并：原「比较意图」+「决策意图」并入品牌意图
  - 例子追加：「蔚来 vs 特斯拉」「该不该买蔚来」「蔚来值不值得」
- **行业意图**：「30 万级纯电 SUV 推荐」「高端纯电轿车」「换电模式优势」「智能驾驶系统」「纯电车保值率」
- **区域意图**：「北京 30 万纯电 SUV 推荐」「上海蔚来 ET5」「广州新能源车推荐」
...
3. 「蔚来 vs 理想」（V1.3 合并到品牌意图，1 角色+1 场景+比较阶段+8 词+3 竞品）→ 弹性区 1.2-1.5x
