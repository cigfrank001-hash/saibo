...
description: 品牌意图选词助手。输入品牌+行业+产品，按 4 阶段流程输出 N 个最小意图单元（每个 = 1 角色+1 场景+1 阶段+5 词），对接迪光 GEO 报价表 V1.3。用于"按品牌出最小意图清单"或"为新 GEO 客户准备 5 词起步包"。
---

# 品牌意图选词助手 Skill

**功能**：根据品牌信息，输出 N 个最小意图单元（5 词起步包），可直接对接迪光 GEO 报价表 V1.3
**作者**：小赛博
...
| 报价表 V1.3 5 维度框架 | 5 步判定 SOP |
| 报价表 V1.3 强相关原则 | 选词 SOP |

| 下游 | 对接 |
|------|------|
| 迪光 GEO 对外报价表 V1.3 | Stage 4 输出 = SKU 输入 |
